...
import com.staircase13.apperta.service.exception.*;
...
public class EhrRecordService {
    ...
    public EhrQueryResultDto executeDirectQuery(EhrRecordSession ehrRecordSession, EhrQueryDto ehrQueryDto)
        ...
        if (selectColumns == null || selectColumns.size() == 0) {
            errors.add("selectColumns is empty");
        }
        if (whereclauses == null || whereclauses.size() == 0) {
            errors.add("whereClauses is empty");
        }
        if (StringUtils.isEmpty(ehrRecordSession.getEhrId())) {
            errors.add("ehr could not be found");
        }
        ...
        if (ehrQueryDto.getStartDateTime() > 0 ) {
            LocalDateTime date = LocalDateTime.ofEpochSecond(ehrQueryDto.getStartDateTime(),0, ZoneOffset.UTC);
            sb.append(" and a/context/start_time > '"+ date + "'");
        }

        if (ehrQueryDto.getEndDateTime() > 0) {
            LocalDateTime date = LocalDateTime.ofEpochSecond(ehrQueryDto.getEndDateTime(),0, ZoneOffset.UTC);
            sb.append(" and a/context/start_time < '"+ date + "'");
        }
    ...
    public EhrQueryResultDto executeTemplatedQuery(EhrRecordSession ehrRecordSession, EhrQueryDto ehrQueryDto)
        ...
        if (ehrQueryDto.getStartDateTime() > 0 ) {
            LocalDateTime date = LocalDateTime.ofEpochSecond(ehrQueryDto.getStartDateTime(),0, ZoneOffset.UTC);
            whereClause.append(" and a/context/start_time > '"+ date + "'");
        }
    ...
    public MultiQueryResultDto executeMultipleQueries(EhrRecordSession ehrRecordSession, MultiEhrQueryDto multiQueryDto)
        ...
        for (String key : queryKeys) {
            ...
            try {
                ...
                queryResultDto = new EhrQueryResultDto("{\"Error\" : \"" + queryException.getMessage() + "\"}",0,0);
            }
    ...
    private void checkRequiredCompositionContextFields(Map<String, List<Map<String,String>>> composition, IdentifiedParty committer, long recordDate) {
        ...
        // This could be expanded to include more context if appropriate for the application
