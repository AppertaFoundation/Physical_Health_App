...
public class PatientsEndpoint {
    ...
    public PatientsResponse findPatients(@RequestParam int start, @RequestParam int pageSize,
                                         @AuthenticationPrincipal AppertaPrinciple userDetails)
                                            throws InvalidHcpUsernameException, EhrAuthenticationException, EhrOperationException {
        HcpDto hcpDto = hcpService.getProfile(userDetails.getUsername());
        List<ProfileDto> profiles;
        if (hcpDto.getNhsId() != null) {
            profiles = demographicsService.findProfilesByHCPId(hcpDto.getNhsId(), start, pageSize, null);
        } else {
    ...
    public PatientsResponse findPatients(@Valid @RequestBody PatientSearchRequest patientSearchRequest,
        ...
        List<ProfileDto> profiles;
        if (hcpDto.getNhsId() != null) {
            profiles = demographicsService.findProfilesByHCPId(hcpDto.getNhsId(), patientSearchRequest.getStart(), patientSearchRequest.getPageSize(), patientSearchRequest.getSearchTerms());
        } else {
            profiles = Collections.emptyList();
        }
        return PatientsResponse.builder()
                ...
                .searchTerms(patientSearchRequest.getSearchTerms())
                .profiles(profiles)
                .build();
    }
