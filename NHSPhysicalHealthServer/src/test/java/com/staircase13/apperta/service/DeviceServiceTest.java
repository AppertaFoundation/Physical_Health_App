...
public class DeviceServiceTest {
    ...
    @Test
    public void updateOrCreateDevice_createNewDevice() throws AppertaException {
    ...
    @Test
    public void updateOrCreateDevice_failConstraintViolation() {
        DataIntegrityViolationException dataException = new DataIntegrityViolationException("could not execute statement; SQL [n/a]; constraint");
        when(deviceRepository.save(any(Device.class))).thenThrow(dataException);

        try {
            deviceService.updateOrCreateDevice(firstDeviceDto, PERSISTED_USER_NAME2);
            assert(false); // Should not reach this point
        } catch (Exception exception){
            assertThat(exception.getMessage(), is("could not execute statement; SQL [n/a]; constraint"));
        }
    }
    ...
    public void updateOrCreateDevice_failUpdateDeviceNotUsers() {
        ...
        try {
            deviceService.updateOrCreateDevice(firstDeviceDto, PERSISTED_USER_NAME2);
        } catch (RuntimeException | AppertaException rte) {
            threw = true;
        }
        assert threw;
    }
    ...
    @Test
    public void updateOrCreateNotification_createCorrectly() throws AppertaException {
    ...
    @Test
    public void updateOrCreateNotification_updateCorrectly() throws AppertaException {
        when(notificationRepository.save(any(DeviceNotification.class))).thenReturn(persistedNotification);
    ...
    @Test
    public void updateOrCreateNotification_failUpdateWrongUser() throws AppertaException {
        ...
        try {
            ...
            assert(false); // Should not reach this point
        } catch (AppertaException exception){
            assertThat(exception.getMessageCode(), is("apperta.device.notification.wrong_user"));
        }
    ...
    @Test
    public void updateOrCreateNotification_failWrongState() throws AppertaException {
        ...
        try {
            ...
            assertThat(exception.getMessageCode(), is("apperta.device.notification.already_complete"));
        }
