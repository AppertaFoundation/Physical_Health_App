...
import com.staircase13.apperta.ehrconnector.impls.ehr.ethercis.dto.EhrDetailsDto;
...
import static com.staircase13.apperta.ehrconnector.impls.ehr.ethercis.EthercisEhrRestConstants.EHR_ENDPOINT;
import static com.staircase13.apperta.ehrconnector.OpenEhrRestConstants.EHR_SUBJECT_NAMESPACE;

@Component
public class EthercisEhrRecord extends EhrBase implements IEhrRecord {

    private static final Logger LOG = LoggerFactory.getLogger(EthercisEhrRecord.class);

    private EthercisEhrConnector ehrConnector;


    @Autowired
    public EthercisEhrRecord(EthercisEhrConnector ehrConnector) {
    ...
    public EhrDetailsDto findEhrStatusByUserName(String userName) throws EhrOperationException, EhrAuthenticationException {
        ...
        try {
            ...
            if (statusCode >= 500 && statusCode < 600) {
                throw new EhrOperationException("EthercisEhrRecord findEhrStatusByUserName server issue: ", rre);
            }
...
//            rre.getResponseBodyAsString()
    ...
    public EhrDetailsDto createEhrForUserName(String userName, IdentifiedParty committer) throws EhrOperationException, EhrAuthenticationException {
        ...
        try {
            ...
            int statusCode = rre.getRawStatusCode();
            if (statusCode >= 500 && statusCode < 600) {
                throw new EhrOperationException("EthercisEhrRecord createEhrForUserName server issue: ", rre);
            }
            // TODO:  handle getting response body and returning for failed validation
//            rre.getResponseBodyAsString()
            switch (rre.getRawStatusCode()) {
