...
public class UserService {
    ...
    public ProfileDto fetchUserDemographics(String username) throws EhrGenericClientException, EhrAuthenticationException {
    ...
    public ProfileDto updateUserDemographics(ProfileDto profileDto, String username, @Nullable IdentifiedParty identifiedParty)
        ...
        if (userOptional.isPresent()) {
            ...
            // TODO: change this as we should already know from endpoints if username == patient
            if (userOptional.get().getRole() == Role.PATIENT) {
                ...
                LOGGER.warn("Demographics update requested on non-patient record. Username: " + username);
            }
        } else {
    ...
    public List<UserDto> getUsers() {
        return appertaUserRepository.findAll().stream().map(u -> mapUserToDto(u)).collect(Collectors.toList());
    }
