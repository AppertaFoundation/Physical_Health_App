...
public class EhrSessionTest {
    ...
    public void startSession_success() throws Exception {
        ...
        ClientHttpRequestExecution execution = (httpRequest, bytes) -> {
            ...
            List<String> sessionHeaders = httpRequest.getHeaders().get(SESSION_ID);
            assertThat(sessionHeaders, Matchers.contains(TestConstants.SESSION_ID));
            return null;
        };
    ...
    @Test
    public void endSession_deleteRemote_remoteFails_deleteRegardless() throws Exception {

        startSession_success();

        doThrow(new RestClientException("oh dear")).when(restTemplate).delete(TestConstants.EHR_URL + SESSION_ENDPOINT);
